...
- **TLS / mTLS** — server-cert verification with hostname checks against the cert's SAN/CN (auto-derived from the URL, or overridden via `NatsOptions.expectedHostname` for IP-dialled or SNI-proxied connections); client cert/key for mutual TLS; TLS ≤ 1.2 cipher allow-list via `NatsOptions.tlsCiphers` (OpenSSL syntax)
